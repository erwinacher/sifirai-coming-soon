...
const evalCommand = (cmd: string, instance: Terminal.Instance) => {
  ...
  const commandArguments =
    argMatches === null
      ? []
      : argMatches.map((element) => element.replace(/(^['"]|['"]$)/g, ""));
  const commandInstance = instance.settings.commands[command];

  if (commandInstance) {
    if (
      commandInstance.argDescriptions &&
      commandInstance.argDescriptions.length > 0 &&
      commandArguments.length === 0
    ) {
      print(
        `Usage: ${command} ${commandInstance.argDescriptions
          .map((arg) => `[${arg}]`)
          .join(" ")}`
      );
    } else {
    ...
    dispatchEvent(
      TerminalEvent.ON_COMMAND_NOT_FOUND,
